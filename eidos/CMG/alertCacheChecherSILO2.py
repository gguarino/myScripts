...
if found:
    ...
    for loid,timeSta in invalidQueries.items():
        ...
        urls = webcacheUrl.split(",")
        for myUrl in urls:
            urlToGet = "http://%s:9601/debug/utils/alertcache.jsp?action=reload&id=%s" % (myUrl, loid)
            errMess = "FIXED!"
        ...
        thisError = "%s - %s - %s - %s\n" % (errorTime, loid, queryPath, myErrMess)
